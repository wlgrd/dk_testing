...
class NRF:
    ...
    def set_dut_production_mode_settings(
        self,
        standard="",
        ch="",
        bw="",
        ...
        debugPrint("\nSetting DUT in production mode\n")
        self.dut_write("nrf_wifi_prod tx 0")
        self.dut_write("nrf_wifi_prod rx 0")
        self.dut_write("nrf_wifi_prod tx_pkt_rate -1")
        self.dut_write("nrf_wifi_prod tx_pkt_mcs -1")
        self.dut_write("nrf_wifi_prod tx_pkt_tput_mode 0")
        # self.dut_write('nrf_wifi_prod rf_params 0000000000002C00000000000000006060606060606060600000000050EC000000000000000000000000')
        self.set_rf_params()
        self.dut_write("nrf_wifi_prod phy_calib_rxdc 1")
        self.dut_write("nrf_wifi_prod phy_calib_txdc 1")
        self.dut_write("nrf_wifi_prod phy_calib_txpow 1")
        self.dut_write("nrf_wifi_prod phy_calib_rxiq 1")
        self.dut_write("nrf_wifi_prod phy_calib_txiq 1")
        # self.dut_write('nrf_wifi_prod phy_calib_dpd 1')
        if "tx" in test:
            if (standard == "11n") or (standard == "11ac"):
                self.dut_write("nrf_wifi_prod tx_pkt_len 4096 ")
            elif (standard == "11a") or (standard == "11b") or (standard == "11g"):
                self.dut_write("nrf_wifi_prod tx_pkt_len 1024")
            elif standard == "11b":
                self.dut_write("nrf_wifi_prod tx_pkt_len 1024")
                if preamble.lower() == "short":
                    self.dut_write("nrf_wifi_prod tx_pkt_preamble 0")  # SHORT
                elif preamble.lower() == "long":
                    self.dut_write("nrf_wifi_prod tx_pkt_preamble 1")  # LONG
    ...
    def set_dut_channel(self, ch):
        ...
        self.dut_write("nrf_wifi_prod tx 0")
        self.dut_write("nrf_wifi_prod rx 0")
        res = self.dut_write_read(f"nrf_wifi_prod chnl_primary {ch}")
    ...
    def get_phy_stats(self):
        res = self.dut_write_read("nrf_wifi_prod get_stats")
        return res
    ...
    def compute_per(self, modulation, dr, ch, streams, standard, tester, dut, equipment, wave_type):
        ...
        end_time = time.time()
        if (
            ("LMAC Initialization Failed" in res)
            or ("SIOCSIFFLAGS" in res)
            or ("cut here" in res)
            or ("Intf not ready for 1000ms" in res)
            or ("Spurious interrupt received" in res)
            or ("RPU_WIFI_HAL" in res)
        ):
            ...
            if int(actual_pkt_cnt) < 100:
                ...
                if int(ch) < 15:
                    if "iqxel" in tester.lower():
                        per = round(
                            ((num_of_pkts_from_vsg + 2 - crc32_pass_cnt) * 100) / float(num_of_pkts_from_vsg + 2)
                        )
                    else:
                        per = round(
                            ((num_of_pkts_from_vsg + 2 - crc32_pass_cnt) * 100) / float(num_of_pkts_from_vsg + 2)
    ...
    def set_dut_datarate(self, data_rate, standard):
        ...
        if "MCS" in data_rate:
			...
			if [ $p -ne "-1" ]
			then
				nrf_wifi_prod tx_pkt_rate -1" > /sys/kernel/debug/img/wlan/conf
			fi 
			"""
            # res=self.dut_write_read(op_str)
            # debugPrint( res )
            res = self.dut_write("nrf_wifi_prod tx 0")
            debugPrint(res)
            if standard == "11n":
                res = self.dut_write("nrf_wifi_prod tx_pkt_tput_mode 1")
            elif standard == "11ac":
                res = self.dut_write("nrf_wifi_prod tx_pkt_tput_mode 2")

            self.dut_write("nrf_wifi_prod tx_pkt_rate -1")
            self.dut_write("nrf_wifi_prod tx_pkt_mcs " + data_rate.lower().split("mcs")[-1])
        else:
            op_str = """q=`cat /sys/kernel/debug/img/wlan/conf | grep tx_pkt_mcs | cut -f2 -d'='`
			if [ $q -ne "-1" ]
			then	
				nrf_wifi_prod tx_pkt_mcs -1" > /sys/kernel/debug/img/wlan/conf
			fi
			"""
            # res=self.dut_write_read(op_str)
            # debugPrint( res )
            ...
            self.dut_write("nrf_wifi_prod tx_pkt_rate " + data_rate)
    ...
    def set_dut_txpower(self, txp):
        ...
        self.dut_write("nrf_wifi_prod tx 0")
        # time.sleep(1)
        self.dut_write("nrf_wifi_prod tx_power " + txp)
        # time.sleep(1)
        self.dut_write("nrf_wifi_prod tx 1")
        # time.sleep(1)
    ...
    def set_rf_params(self, xo="2C"):
        if os.path.exists("xo_val_board_" + board_num + ".txt"):
            ...
            if xo == "":
                xo = "2C"
        self.dut_write(
            "nrf_wifi_prod rf_params 000000000000"
            + xo
            + "00000000000000006060606060606060600000000050EC000000000000000000000000"
        )
    ...
    def set_dut_xo(self, xo="2C"):
        ...
        self.dut_write("nrf_wifi_prod tx 0")
        time.sleep(0.5)
        # self.dut_write('nrf_wifi_prod rf_params 00000000000000000000000000006060606060606060600000000050EC000000000000000000000000')
        self.dut_write(
            "nrf_wifi_prod rf_params 000000000000"
            + xo
            + "00000000000000006060606060606060600000000050EC000000000000000000000000"
    ...
    def set_dut_rfconf(self, release):
        ...
        if os.path.exists("rf_conf_" + release):
            ...
            rfconf_string = (
                rfconf_string_read.split("rf_conf =")[1]
                .split("\r\n")[0]
                .replace(" ", "")
                .replace(
                    "",
        ...
        self.dut_write("nrf_wifi_prod rf_conf " + rfconf_string)
        return rfconf_string

    def set_production_calib_txpower(self, txp):
        # print"\nSetting TX Power ",txp
        debugPrint("\nSetting TX Power " + str(txp))
        self.dut_write("nrf_wifi_prod set_tx_power " + txp)
    ...
    def dut_down_up(self, action="down", ch="36"):
        ...
        if action == "down":
            ...
            # self.dut_write('nrf_wifi_prod tx=0 ')
            self.dut_write("nrf_wifi_prod rx 0")
            self.dut_write("nrf_wifi_prod rx 1")
    ...
    def dut_access(self, dut_mgmt_ip="", dut_username="", dut_password=""):
        ...
        if 1:
            ...
            dut_obj = serial.Serial(
                self.com_port, "115200", parity=serial.PARITY_NONE, bytesize=serial.EIGHTBITS, timeout=4
            )
            return "SUCCESS"
        # except Exception as e:
        # err_args=e.args
    ...
    def load_dut(
        ...
        self.set_dut_production_mode_settings(
            standard=standard,
            ch=channel,
            stbc=stbc,
            bw=bw,
            gi=gi,
            coding=coding,
            greenfield_mode=greenfield_mode,
            preamble=preamble,
            test=test,
            action="only_enable",
        )
