...
def get_optimal_xo():
    ...
    while True :
        for xo_val in xo_list:
            ...
            return_data=get_tx_stats_from_vsa(txp,dtr,channel,streams,standard,bw,equipment,cable_loss_1x1,cable_loss_2x2)
            sym_clk_err = abs(return_data[6])
            if sym_clk_err <= 0.2:
                found = 1
                break
            else:
        ...
        if iter_num > 3 and prev_xo_list == curr_xo_list:
            optimal_xo = hex(min(xo_val_dict, key=xo_val_dict.get)).zfill(2).replace('0x','')
            print colored("Ther is some issue with the board.Not able to converge best possible XO", 'red', attrs=['reverse', 'blink','bold'])
            print colored("Got best possibel Symbol Clock Error as "+str(sym_clk_err)+" at "+str(xo_to_set), attrs=['reverse','bold'])            
            fout = open('xo_val_board_'+board_num+'.txt', 'w')
            fout.write(optimal_xo)
            fout.close()
            return optimal_xo
